	...
	class Buyer extends CI_Controller{
		...
		public function placeOrder($Product_id){
			...
			$this->form_validation->set_rules('quantity','Quantity','required|callback_check_quantity_not_exists['.$Product_id.']');
		...
		public function check_quantity_not_exists($quantity,$Product_id){
			...
			$this->form_validation->set_message('check_quantity_not_exists','Quantity availabilty is low');
			
			if($this->Crop_model->check_quantity_not_exists($quantity,$Product_id)){
				return false;
			}
			else{
				return true;
